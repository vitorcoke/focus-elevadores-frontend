...
import { countryOptions, normalizeCountryCode } from "../../../constants/countryOptions";
...
const EditNoticies: React.FC<EditNoticiesProps> = ({ noticies, setNoticies }) => {
  ...
  const [form, setForm] = useState({ ...noticies, country: normalizeCountryCode(noticies.country) });
  const [screen, setScreen] = useState<Screen[]>([]);
  const [selectedScreens, setSelectedScreens] = useState<string[]>([]);
  const [status, setStatus] = useState<"success" | "error" | null>(null);

  useEffect(() => { setForm({ ...noticies, country: normalizeCountryCode(noticies.country) }); setSelectedScreens(noticies.screen_id || []); }, [noticies]);
